# AI for Doctors & Medical Practices — `/en/industries/ai-for-medical-practices`
...
**Page type (§9):** Service / sector landing (medical practices) — MOFU
...
|---|---|
| HTTP status | 200 |
| Title (rendered, incl. `\| AI Makers`) | `AI for Doctors & Medical Practices \| AI Makers` — **46** chars |
| Meta description | **146** — in budget |
| `<h1>` count | 1 |
| `<h2>` / `<h3>` | 8 / 14 |
| Body words (header/footer stripped) | 745 |
| JSON-LD emitted | `BreadcrumbList`, `FAQPage` |
| hreflang | reciprocal fr/en/x-default ✅ |
| In `sitemap.xml` | yes ✅ |
| Canonical | `https://aimakers.fr/en/industries/ai-for-medical-practices` ✅ |
...
- **Title is well inside budget** at 46 chars including the auto-appended `| AI Makers` — no double brand suffix, which the masters' reconciliation notes flag as a repeat offence.
- **745 words** of substantive body copy, comfortably clear of thin.
