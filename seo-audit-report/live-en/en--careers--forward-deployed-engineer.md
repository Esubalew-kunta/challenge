# Forward Deployed Engineer (role) — role page — `/en/careers/forward-deployed-engineer`
...
|---|---|
| HTTP status | 200 |
| Title (rendered, incl. auto-appended brand) | `Forward Deployed Engineer: AI Makers Careers \| AI Makers` — **56** chars ✅ |
| Meta description | **168** — over the 160 budget 🟡 |
| `<h1>` | 1 — `Forward Deployed Engineer : The role at the centre of the game` |
| `<h2>` / `<h3>` | 5 / 13 |
| Body words (header/footer stripped) | 658 |
| JSON-LD emitted (verified in rendered HTML) | `BreadcrumbList`, `JobPosting` |
| hreflang | reciprocal fr/en/x-default ✅ |
| In `sitemap.xml` | yes ✅ |
| Canonical | `https://aimakers.fr/en/careers/forward-deployed-engineer` ✅ |
...
🟡 **Meta description is 168 chars, 8 over the 160 budget.** The tail truncates in search results; move the strongest hook forward and trim at source.
...
- **Title 56 chars** including the auto-appended brand — inside budget, and free of the double-suffix defect that affects the parent hub.
- **658 words**: substantive for a role page, not a pasted job spec.
