# GTM / Growth Manager — role page — `/en/careers/gtm-growth-manager`
...
|---|---|
| HTTP status | 200 |
| Title (rendered, incl. auto-appended brand) | `GTM & Growth Manager: AI Makers Careers \| AI Makers` — **51** chars ✅ |
| Meta description | **175** — over the 160 budget 🟡 |
| `<h1>` | 1 — `GTM Growth Manager : You open the pipeline, end to end` |
| `<h2>` / `<h3>` | 5 / 13 |
| Body words (header/footer stripped) | 651 |
| JSON-LD emitted (verified in rendered HTML) | `BreadcrumbList`, `JobPosting` |
| hreflang | reciprocal fr/en/x-default ✅ |
| In `sitemap.xml` | yes ✅ |
| Canonical | `https://aimakers.fr/en/careers/gtm-growth-manager` ✅ |
...
🟡 **Meta description is 175 chars, 15 over the 160 budget.** The tail truncates in search results; move the strongest hook forward and trim at source.
...
- **Title 51 chars** including the auto-appended brand — inside budget, and free of the double-suffix defect that affects the parent hub.
- **651 words**: substantive for a role page, not a pasted job spec.
