...
export async function POST(request: Request) {
  try {
    ...
    // Generate a lead ID
    const leadId = `diag_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
    ...
    if (!captured.ok) {
      ...
      const relay = await relayToN8n(process.env.N8N_DIAGNOSTIC_WEBHOOK_URL, {
        leadId,
      ...
      if (!relay.ok) {
        console.error(
          `[DIAGNOSTIC] ni OS (${captured.reason}) ni n8n (${relay.reason}) : ${data.email}`,
        );
        return NextResponse.json(
          {
            error:
              "Votre diagnostic n'a pas pu être envoyé. Merci de réessayer dans un instant.",
          },
          { status: 502 },
        );
      }
      console.warn(`[DIAGNOSTIC] OS indisponible (${captured.reason}) — relayé à n8n en direct`);
      return NextResponse.json({ success: true, leadId });
    }

    after(() => requestLeadNotification(captured.id));

    console.log(`[DIAGNOSTIC] New lead: ${data.email} | Score: ${data.totalScore}/20 | Level: ${data.level} | Sector: ${data.sector}`);
